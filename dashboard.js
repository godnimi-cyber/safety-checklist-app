...
(function () {
  ...
  function renderOverdue_(data) {
    ...
    /* F4(사용자 실기기 리포트) — 390px 에서 3줄(53px), 마지막 줄에 "니다." 2글자만 남는
       고아줄이라 조치할 항목보다 설명이 먼저 시선을 잡았다. 버튼 라벨(미점검확정·공사취소)이
       이미 화면에 있어 정보 손실 없이 줄인다: "재등록하면 다시 작성할 수 있고, 미점검확정은
       점검하지 않았음을, 공사취소는 공사가 없어졌음을 남기고 목록에서 내립니다" →
       "재등록하면 다시 작성할 수 있습니다". */
  ...
  function fetchDashboard(okMsg, bg) {
    ...
    setBusy_(true);
    showBanner_('');
    /* 재시도 고지는 **이번 세대(g)일 때만** 쓴다 — 늦게 도착한 옛 요청의 재시도가
       최신 조회 화면에 배너를 덮어쓰면 사용자는 방금 누른 조회가 헤매는 줄 안다. */
    getJson(url, function (nth, total) {
